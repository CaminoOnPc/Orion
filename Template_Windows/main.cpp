...
MainComponent wimain;								// Wicked Engine Main Runtime Component
                     ...
                     _In_ int       nCmdShow)
{
    ...
    wimain.infoDisplay.active = true;
    wimain.infoDisplay.watermark = false;
    wimain.infoDisplay.resolution = true;
    wimain.infoDisplay.fpsinfo = true;
    ...
    wimain.ActivatePath(&myGame); // Register your game to the application. It will call Start(), Update(), Render(), etc. from now on...

    wiScene::LoadModel("sponza.wiscene"); // Simply load a model into the current global scene
   ...
   // wiRenderer::ClearWorld(); // Delete every model, etc. from the current global scene
    ...
    //wiScene::LoadModel(scene2, "sponza.wiscene"); // Load model into a separate scene
	...
	while (msg.message != WM_QUIT)
	{
		if (PeekMessage(&msg, NULL, 0, 0, PM_REMOVE)) {
            ...
            wimain.Run(); // run the update - render loop (mandatory)
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
   ...
   wimain.SetWindow(hWnd); // assign window handle (mandatory)
